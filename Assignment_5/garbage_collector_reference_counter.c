...
void bifurcate_memoryblock(struct block *toallocate, int user_demand)
{
    ...
    remaining->Reference_count = 0;
    toallocate->size = user_demand;
    toallocate->status = Allocated;
    toallocate->next = remaining;
}

void increment_Reference_count(struct block *allocated_block)
{
    allocated_block->Reference_count++;
}

void decrement_Reference_count(struct block *allocated_block)
{
    if (allocated_block->Reference_count > 0)
    {
        allocated_block->Reference_count--;
    }
}
...
{
    struct block *current_block = list; // temp pointer to point the linked list of allocated memory blocks

    while (current_block != NULL)
    {
        if (current_block->status == Allocated && current_block->Reference_count == 0)
        {
            current_block->status = Free;
            printf("Garbage collected block of size %d\n", current_block->size);
        }
    ...
    while (current_block != NULL && !is_request_fulfilled)
    {
        if (current_block->status == Free)
        {
            if (current_block->size == user_demand)
            {
                ...
                increment_Reference_count(current_block);
...
void de_allocatememory(void *to_free)
{
    if ((void *)list <= to_free && to_free <= (void *)list + HEAPSIZE)
    {
        ...
        decrement_Reference_count(block_to_free);
        garbage_collect(); //
